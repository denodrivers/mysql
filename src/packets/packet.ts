...
export class ReceivePacket {
    ...
    async parse(reader: Reader): Promise<ReceivePacket> {
        ...
        switch (this.body.buffer[0]) {
            case 0x00: this.type = "OK"; break;
            case 0xff: this.type = "ERR"; break;
            case 0xfe: this.type = "EOF"; break;
            default: this.type = "RESULT"; break;
        }
        ...
        debug(() => {
            ...
            log.debug(`receive: ${result.nread}B, size = ${this.header.size}, no = ${this.header.no} \n${byteFormat(data)}`);
        });
